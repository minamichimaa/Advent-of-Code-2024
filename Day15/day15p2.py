...
def checkBrokenBox(grid):
    for line in grid:
        ...
        if '[.' in stringified or '.]' in stringified or ']]' in stringified or '[[' in stringified:
            return True
    ...
    # box
        ...
        if currentObs == '.':
            ...
            for i in range(len(boxStack)-1, 0, -1):
                grid[int(boxStack[i].real)][int(boxStack[i].imag)] = grid[int(boxStack[i-1].real)][int(boxStack[i-1].imag)]
            grid[int(coords.real)][int(coords.imag)] = '.'
            # update coords
            coords = moveTo
    else:
        ...
        while len(toVisit):
            ...
            boxes[currentObsPos] = grid[int(currentObsPos.real)][int(currentObsPos.imag)]
